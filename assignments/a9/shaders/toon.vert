...
uniform bool outline;   /*flag to indicate outline rendering*/
uniform float outlineScale; /*scale of the outline*/
...
void main() {
    ...
    vtx_uv = uv.xy;
    vtx_tangent = normalize(worldTangent.xyz);

    // Inflate the position for the outline
    if (outline) {
        worldPos.xyz += normalize(worldNormal.xyz) * outlineScale;
    }
